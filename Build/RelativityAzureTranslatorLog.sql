<script>
	<name>Azure Translator Log</name>
	<version>1</version>
	<key>ec584caa-dedd-497e-90bb-49c1ed8d1703</key>
	<description>This Relativity script returns Azure Translator log table.</description>
	...
	<input>
		...
		<field id="LogField" name="Azure Translator Log Field:">
			<filters>
				<type>4</type>
				<required>true</required>
			</filters>
	...
	<action returns="table" timeout="600">
		...
		CREATE TABLE [#data] (
			...
			[email] NVARCHAR(MAX),
			[timestamp] DATETIME,
			[language_from] NVARCHAR(MAX),
			[language_to] NVARCHAR(MAX),
		...
		DECLARE @language_from NVARCHAR(MAX);
		DECLARE @language_to NVARCHAR(MAX);
		...
		BEGIN
			...
			BEGIN
				...
				SET @language_from = LEFT(@hlp, CHARINDEX(@separator_value, @hlp) - 1);
				SET @hlp = RIGHT(@hlp, LEN(@hlp) - CHARINDEX(@separator_value, @hlp));
				--PRINT @language_from;

				SET @language_to = LEFT(@hlp, CHARINDEX(@separator_value, @hlp) - 1);
				SET @hlp = RIGHT(@hlp, LEN(@hlp) - CHARINDEX(@separator_value, @hlp));
				--PRINT @language_to;
				...
				IF (#Month# = '00' OR #Month# = FORMAT(CAST(@timestamp AS DATETIME), 'MM')) AND (#Year# = '0000' OR #Year# = FORMAT(CAST(@timestamp AS DATETIME), 'yyyy'))
					INSERT INTO [#data] ([id], [engine], [email], [timestamp], [language_from], [language_to], [chars_from], [chars_to]) VALUES (@id, @engine, @email, @timestamp, @language_from, @language_to, @chars_from, @chars_to);
